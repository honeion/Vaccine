...
<v-card style="width:300px; margin-left:5%;" outlined>
    ...
    <v-card-title v-if="value._links">
        HospitalManagement # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
    </v-card-title >
    <v-card-title v-else>
        HospitalManagement
    </v-card-title >
    ...
    methods: {
      ...
      async save(){
        try{
          ...
          if(this.isNew){
            temp = await axios.post(axios.fixUrl('/hospitalManagements'), this.value)
          }else{
