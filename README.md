...
- [분석/설계](#분석설계)
  ...
  - [Event Storming 결과](#event-storming-결과)
    ...
    - [폴리시 부착 (괄호는 수행주체, 폴리시 부착을 둘째단계에서 해놔도 상관 없음. 전체 연계가 초기에 드러남)](#폴리시-부착-괄호는-수행주체-폴리시-부착을-둘째단계에서-해놔도-상관-없음-전체-연계가-초기에-드러남)
...
비기능적 요구사항
1. 트랜잭션
    - 백신이 할당되지 않으면 병원에 등록되지 않는다. `Sync 호출`

1. 장애격리
    - 백신 관리 기능이 수행되지 않더라도 신청은 365일 24시간 가능해야한다. `Async (event-driven)`, `Eventual Consistency`
    -  백신 예약이 많아 백신 할당이 급증하여 장애가 발생하면 병원에 등록되는 것은 잠시 뒤에 처리되도록 한다 `Circuit breaker`, `fallback`
1. 성능
    - 고객은 백신예약 현황을 언제든지 확인할 수 있어야한다 . `CQRS`
    - 예약상태가 변경되면 알림을 줄 수 있어야한다 `Event driven`
